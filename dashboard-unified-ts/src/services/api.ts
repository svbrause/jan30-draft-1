...
export async function fetchProviderByCode(providerCode: string): Promise<Provider> {
...
export async function fetchContactHistory(
  ...
  if (!response.ok) {
    if (response.status === 414) {
      console.warn('URI too long error (414) - Contact history temporarily unavailable');
      return [];
    }
    const errorData = await safeJsonParse(response).catch(() => ({}));
  ...
  return allRecords.map((record: AirtableRecord) => {
    const fields = record.fields || {};
    const linkedRecordArray = fields[linkField] || [];
    const leadId = Array.isArray(linkedRecordArray) && linkedRecordArray.length > 0 
      ? linkedRecordArray[0] 
      : null;
    ...
    else if (outcome.includes('no-show') || outcome.includes('no show')) normalizedOutcome = 'no-show';
    ...
    else if (outcome.includes('cancelled') || outcome.includes('canceled')) normalizedOutcome = 'cancelled';
...
export async function createLeadRecord(
  ...
  if (!response.ok) {
    const errorData = await safeJsonParse(response).catch(() => ({}));
    throw new Error(errorData.error?.message || errorData.message || 'Failed to create lead');
  }
...
export async function updateFacialAnalysisStatus(
  ...
  const airtableStatus = newStatus === 'not-started' || !newStatus ? '' : newStatus;
