 ...
 */
function mapAirtableStatus(fields: Record<string, any>): 'new' | 'contacted' | 'scheduled' | 'converted' {
  const status = (fields['Status'] || '').toLowerCase();
  
  if (status.includes('converted') || status.includes('booked') || status.includes('patient')) {
    return 'converted';
  }
...
function formatPatientSource(sourceValue: string | null | undefined): string {
  ...
  } else if (source === 'treatment qr' || (source.includes('treatment') && source.includes('qr'))) {
 ...
 */
function determinePriority(fields: Record<string, any>): 'high' | 'medium' | 'low' {
  ...
  if (photosViewed > 5 || (lastContact && new Date(lastContact) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000))) {
    return 'high';
  }
 ...
 */
export function mapRecordToClient(record: AirtableRecord, tableName: string): Client {
  ...
  const client: Client = {
    ...
    zipCode: fields['Zip Code'] || fields['Zip'] || fields['Postal Code'] || null,
    ...
    dateOfBirth: tableName === 'Patients' 
    ...
    areas: tableName === 'Patients' ? null : (Array.isArray(fields['Areas']) ? fields['Areas'] : null),
    aestheticGoals: (() => {
    ...
    concernsExplored: tableName === 'Patients' ? null : (Array.isArray(fields['Concerns Explored']) ? fields['Concerns Explored'] : null),
    ...
    source: (() => {
      if (tableName === 'Web Popup Leads') {
        ...
        const sourceValue = fields['Source'] || fields['source'] || fields['SOURCE'] || '';
    ...
    allIssues: tableName === 'Patients' ? (fields['Name (from All Issues) (from Analyses)'] || '') : '',
    ...
    areasOfInterestFromForm: tableName === 'Patients'
      ? (Array.isArray(fields['Areas of Interest (from Form Submissions)'])
          ...
          : (typeof fields['Areas of Interest (from Form Submissions)'] === 'string' 
              ? fields['Areas of Interest (from Form Submissions)'] 
              : ''))
      : '',
