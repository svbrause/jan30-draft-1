...
export function applyFilters(clients: Client[], filters: FilterState, searchQuery: string): Client[] {
  ...
  if (filters.source) {
    ...
    filtered = filtered.filter(client => {
      ...
      if (filters.source === 'Online Treatment Finder') {
        return client.tableSource === 'Web Popup Leads' || 
  ...
  if (filters.ageMin !== null || filters.ageMax !== null) {
    filtered = filtered.filter(client => {
      ...
      const age = typeof client.age === 'string' ? parseFloat(client.age) : Number(client.age);
