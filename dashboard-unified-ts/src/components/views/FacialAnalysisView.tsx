...
export default function FacialAnalysisView() {
  ...
  const [showPatientIssues, setShowPatientIssues] = useState<typeof clients[0] | null>(null);
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      await preloadVisiblePhotos(paginatedClients, provider.id);
  ...
  const processedClientsData = useMemo(() => {
    return paginatedClients.map(client => {
      ...
      // Parse issues
      const allIssues = isPatient && client.allIssues
        ? (typeof client.allIssues === 'string' ? client.allIssues.split(',').map(i => i.trim()).filter(i => i) : [])
        : [];
      const interestedIssues = isPatient && client.interestedIssues
        ? (typeof client.interestedIssues === 'string' ? client.interestedIssues.split(',').map(i => i.trim()).filter(i => i) : [])
        : [];
      const whichRegions = isPatient && client.whichRegions ? client.whichRegions : '';
      const skinComplaints = isPatient && client.skinComplaints ? client.skinComplaints : '';
      const processedAreas = isPatient && client.processedAreasOfInterest
        ? (typeof client.processedAreasOfInterest === 'string' ? client.processedAreasOfInterest.split(',').map(a => a.trim()).filter(a => a) : [])
        : [];

      const hasFacialAnalysisData = isPatient && (
        allIssues.length > 0 || 
      ...
      const findings = typeof client.concerns === 'string' 
        ? client.concerns.split(',').map(c => c.trim()).filter(c => c)
        : (Array.isArray(client.concerns) ? client.concerns : []);
      const interests = client.goals || [];

      const cardDate = client.lastContact || client.createdAt || new Date().toISOString();
      const borderColor = getFacialStatusBorderColor(client.facialAnalysisStatus);
        ...
        {processedClientsData.map(({ 
          ...
          return (
            <div
              ...
              onDragStart={(e) => client.tableSource === 'Patients' && handleDragStart(e, client.id)}
              ...
              <div className="facial-card-content">
                <div className="facial-card-photo-container">
                  <div className="facial-card-photo">
                    {clientPhotos[client.id] ? (
                      <img src={clientPhotos[client.id]} alt={client.name} loading="lazy" className="facial-card-photo-img" />
                    ) : (
                  ...
                  <div className="facial-card-status">
                    <span className="status-badge status-badge-base" style={{
                      background: getFacialStatusBorderColor(client.facialAnalysisStatus),
                    }}>
                ...
                <div className="facial-card-details">
                  ...
                  <div className="facial-card-info">
                    <div className="facial-card-email">{client.email || 'No email'}</div>
                    {client.phone && <div className="facial-card-phone">{client.phone}</div>}
                    <div className="facial-card-date">{formatRelativeDate(cardDate)}</div>
                  </div>
                </div>
                <button
                    ...
                    <>
                      {allIssues.length > 0 && (
                        <div className="facial-card-section">
                          <div className="facial-card-section-title">Issues Detected</div>
                          <div className="facial-card-tags">
                            {allIssues.map((issue, i) => (
                              <span key={i} className="facial-tag">{issue}</span>
                            ))}
                          </div>
                        </div>
                      )}
                      <div className="facial-card-section">
                        <div className="facial-card-section-title">Interested Treatments</div>
                        {interestedIssues.length > 0 ? (
                          <div className="facial-card-tags">
                            {interestedIssues.map((issue, i) => (
                              <span key={i} className="facial-tag interest">{issue}</span>
                            ))}
                          </div>
                        ) : (
                      ...
                      {processedAreas.length > 0 && (
                        <div className="facial-card-section">
                          <div className="facial-card-section-title">Focus Areas</div>
                      ...
                      {skinComplaints && (
                        <div className="facial-card-section">
                          <div className="facial-card-section-title">Skin Complaints</div>
                          <div className="facial-card-text">{skinComplaints}</div>
                        </div>
                      )}
                    </>
                  ) : (
                      ...
                      {findings.length > 0 || interests.length > 0 ? (
                        <>
                          {findings.length > 0 && (
                            <div className="facial-card-section">
                              ...
                              <div className="facial-card-tags">
                                {findings.map((f, i) => (
                                  <span key={i} className="facial-tag">{f}</span>
                                ))}
                              </div>
                            </div>
                          )}
                          {interests.length > 0 && (
                            <div className="facial-card-section">
                              <div className="facial-card-section-title">Interests</div>
                              <div className="facial-card-tags">
                                {interests.map((i, idx) => (
                                  <span key={idx} className="facial-tag interest">{i}</span>
                                ))}
                              </div>
                            </div>
                          )}
                        ...
                        <div className="facial-card-section">
                          <div className="facial-card-text facial-card-text-empty">
                            {isPatient ? 'No facial analysis data available yet.' : 'No data recorded.'}
                          </div>
                        </div>
                      )}
                    </>
